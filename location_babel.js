'use strict'; //import {allCharacters} from './character.js'; //import pole všech postav
//console.log (allCharacters);

var fetchPath = 'https://rickandmortyapi.com/api/location/?page=';
var newHtml = '';
everything(); //nelze stáhnout všechna místa najednou, je třeba postupně stáhnout 5 stránek přes API, postupné měnění adresy, ze které se data stahují
...
function getHtml(data) {
  for (var i = 0; i < data.results.length; i++) {
    newHtml += "<div class=\"place\" id=\"".concat(data.results[i].id, "\">\n        <div class=\"name\"> ").concat(data.results[i].name, " </div>\n        <div class=\"type\"> Type: ").concat(data.results[i].type, " </div>\n        <div class=\"dimension\"> Dimension: ").concat(data.results[i].dimension, "</div>\n        </div>");
